...
task 'perform_release' do
  ...
  in_dir(WORKSPACE_DIR) do
    ...
    # Disabled this release check until we have a more mature release
    if false
...
### [v#{ENV['PRODUCT_VERSION']}](https://github.com/realityforge/vecmath/tree/v#{ENV['PRODUCT_VERSION']}) (#{ENV['RELEASE_DATE']})
[Full Changelog](https://github.com/realityforge/vecmath/compare/#{from}...v#{ENV['PRODUCT_VERSION']})
HEADER
...
HEADER
    ...
    stage('GithubRelease', 'Create a Release on GitHub') do
      ...
      client.login
      client.create_release('realityforge/vecmath', tag, :name => tag, :body => changes, :draft => false, :prerelease => prerelease)

      candidates = client.list_milestones('realityforge/vecmath').select {|m| m[:title].to_s == tag}
      unless candidates.empty?
        milestone = candidates[0]
        unless milestone[:state] == 'closed'
          client.update_milestone('realityforge/vecmath', milestone[:number], :state => 'closed')
        end
      end
    end
  end
